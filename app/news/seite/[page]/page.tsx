...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `KI News - Seite ${page}`,
    description: `Seite ${page} der KI News - Aktuelle Nachrichten und Updates aus der Welt der künstlichen Intelligenz`,
    alternates: {
      canonical: `${siteUrl}/news/seite/${page}`,
    },
  }
}

export default async function NewsPagePage({ params }: Props) {
  const page = parseInt(params.page, 10)
  ...
  const { articles, totalPages, currentPage, totalArticles } =
    await getPaginatedNewsArticles(page, 10)

  // If page is out of range, show 404
  if (currentPage !== page || (page > 1 && articles.length === 0)) {
    notFound()
  }
