...
import { getAllCategories, getPaginatedArticlesByCategory } from '@/lib/articles'
...
export async function generateStaticParams() {
  ...
  for (const category of categories) {
    ...
    for (let i = 1; i <= totalPages; i++) {
      if (i > 1) {
        // Skip page 1 as it redirects to base category page
        params.push({
          slug: categorySlug,
          page: String(i),
        })
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Kategorie: ${category} - Seite ${page}`,
    description: `Alle Artikel in der Kategorie ${category} - Seite ${page}`,
    alternates: {
      canonical: `${siteUrl}/kategorie/${slug}/seite/${page}`,
    },
  }
}
...
export default async function CategoryPagePage({ params }: Props) {
  ...
  // Redirect page 1 to base category page
  if (page === 1) {
    redirect(`/kategorie/${slug}`)
  }

  const { articles, totalPages, currentPage, totalArticles } = 
    await getPaginatedArticlesByCategory(category, page, ARTICLES_PER_PAGE)
      ...
      <div className="max-w-7xl mx-auto px-4 py-20">
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight">
            Kategorie: <span className="text-emerald-400">{category}</span>
          </h1>
