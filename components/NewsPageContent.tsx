...
export function NewsPageContent({ articles, totalArticles, currentPage }: NewsPageContentProps) {
  const { t } = useLanguage()
    ...
    <>
      ...
      <div className="mb-12">
        <h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight">
          ...
          <span className="text-emerald-400">{t.news.title.split(' ').slice(-1)[0]}</span>
        </h1>
      ...
      <div className="text-center mt-8 text-slate-400 text-sm">
        {t.news.showing} {articles.length} {t.news.of} {totalArticles} {t.news.newsArticles}
      </div>
    </>
  )
}
